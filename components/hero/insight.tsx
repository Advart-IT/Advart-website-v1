...
const GsapStackScroll: React.FC = () => {
  ...
  const cards: Card[] = [
    {
      imageSrc: "hero/zing/bee-little.webp",
      title: "Beelittle",
      description:
    ...
    {
      imageSrc: "hero/zing/prathiksham.webp",
      title: "Prathiksham",
      description:
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Initial state: first card at 0%, others completely hidden below
      ...
      cards.forEach((_, index) => {
        ...
        // Bring current card up from completely hidden position
        sectionTL.fromTo(currentCard, { yPercent: 120 }, { yPercent: 0, duration: 1 })

        // Push/blur previous card slightly
        sectionTL.to(
          prevCard,
          {
        ...
        // Enable interaction on new card
    ...
    <div className="relative">
      <section className="relative w-full">
        <div ref={wrapRef} className="relative min-h-screen w-full">
          {/* Enhanced centering container */}
          <div className="fixed top-0 left-0 w-full h-screen flex items-center justify-center z-10">
            <div className="relative w-[95%] sm:w-[97%] lg:w-[99%] max-w-7xl mx-auto h-[80vh] sm:h-[70vh] lg:h-[75vh] overflow-hidden">
              ...
              {cards.map((card, index) => (
                <div
                  ...
                  className="absolute inset-0 will-change-transform will-change-filter overflow-hidden"
                  style={{ zIndex: index + 1 }}
                >
                  <div className="relative h-full w-full overflow-hidden rounded-2xl shadow-2xl ring-1 ring-white/10 bg-black text-white">
                    {/* Content properly centered within card */}
                    <div className="h-full flex items-center px-4 sm:px-6 md:px-8 lg:px-12">
                      <div className="max-w-7xl mx-auto w-full">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 items-center">
                          <div className="space-y-4 lg:space-y-6 order-2 lg:order-1">
                            <div className="card-heading">
                              <h1 className="text-xl sm:text-2xl lg:text-4xl xl:text-5xl font-light mb-3 lg:mb-4 leading-tight">
                                {card.title}
                              </h1>
                              <div className="card-description">
                                <p className="text-sm sm:text-base lg:text-lg xl:text-xl font-light leading-relaxed text-gray-200">
                                  {card.description}
                                </p>
                              </div>
                            </div>

                            <div className="card-what-tried space-y-2 lg:space-y-3">
                            ...
                            <div className="card-result space-y-2 lg:space-y-3">
                              <h3 className="text-xs sm:text-sm font-medium text-white">And the result?</h3>
                              <p className="text-gray-300 leading-relaxed text-sm lg:text-base">{card.result}</p>
                            </div>
                          </div>

                          <div className="flex justify-center lg:justify-end order-1 lg:order-2">
                            <div className="card-image relative w-full max-w-xs sm:max-w-sm lg:max-w-md">
                              <img
                                src={card.imageSrc}
                                alt={card.imageAlt ?? `${card.title} showcase`}
                                className="w-full h-auto max-h-[40vh] sm:max-h-[45vh] object-contain rounded-lg shadow-lg mx-auto"
                              />
                            </div>
                          </div>
                        </div>
